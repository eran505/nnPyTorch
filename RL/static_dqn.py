...
import PRB.util as utils
import torch.distributions as td

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
from RL.logger import logger
from RL.logger import create_stats_ordered_dict
...
class DQfD(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        ...
        for it in range(iterations):
            ...
            state_np, action_np, reward, next_state_np, done = replay_buffer.sample(batch_size)
            state = torch.FloatTensor(state_np).to(device)
            ...
            noise = torch.FloatTensor(action_np).data.normal_(0, policy_noise).view(batch_size,1).to(device)
            noise = noise.clamp(-noise_clip, noise_clip)
            res = self.actor_target(next_state)
            next_action = (res + noise).clamp(-self.max_action, self.max_action)
